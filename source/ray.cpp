...
ColorDBL Ray::castShadowRay(const Polygon* fromPolygon, const Polygon* lightsource, const glm::vec3 pointOnSurface) {//Should change lightsource to a lightsource instead of polygon
    //implement function to see if there is an object in the way
    
    //The shadow ray
    Ray shadowRay(pointOnSurface, (lightsource->  -pointOnSurface));
    ...
    if(glm::dot(fromPolygon->normal, lightsource->normal) <=  0){
        ...
        if(pointOnLight != glm::vec3(-100,-100,-100)){
            this->color = this->color.mult(lightsource->getColor());
        }
    }
    return this->color;
}
