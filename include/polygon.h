...
class Object {
public:
    //returns the normal, should be used in the constructor (Might need to be implemented in seperaate child classes "problem in sphere")
    ...
    //Calculate the point where we intersect the plane and then if it is in the polygon and returns that point (use 0 in sphere)
    virtual glm::vec3 PointInObject(const Ray& ray) const = 0;
    ColorDBL color_;
...
public:
    const float errorMargin = 10e-5f; //error margin i guess ;)
...
public:
    ...
    glm::vec3 PointInObject(const Ray& ray) const override;
    glm::vec3 recNormal;
